...
В Kubernetes каждая группа экземпляров одного микросервиса (подов) подчиняется объекту типа Service, который имеет виртуальный IP (ClusterIP) и используется как единая точка доступа к подам, соответствующим селектору. У каждого сервиса есть свое доменное имя типа `service.namespace.svc.cluster.local`. Также у каждого пода есть свой IP, но он часто меняется из-за перезапусков и других причин, поэтому напрямую к IP пода обращаться ненадежно.
В самом начале, когда надо обратиться к какому-то другому сервису, запрос идет по доменному имени стороннего сервиса. Далее DNS-резолвер (в k8s используется по дефолту DNS сервер CoreDNS) по доменному имени сервиса возвращает ClusterIP сервиса и после этого можно делать запрос. Дальше в ход идет kube-proxy. Он запущен на каждой ноде и смотрит за сервисами и их подами. Когда под делает запрос к ClusterIP сервиса, kube-proxy этого сервиса перехватывает его. После просмотра ip и порта места назначения он может идентифицировать ip пода, который обработает этот запрос. Далее kube-proxy заменяет место назначения этого запроса на адрес пода, готового обработать этот запрос. Далее этот запрос доходит до пода, обрабатывается им, и ответ возвращается напрямую по ip пода, который его отправлял.


## 5. Логическая схема БД

<img width="1639" height="784" alt="image" src="https://github.com/user-attachments/assets/377cf165-0d0f-4054-9597-e4ba755b7922" />
...
| **skills_resumes**     | Таблица-связь М:М между навыками и резюме. Хранит уровень мастерства этого навыка в резюме                                              |
| **skills**             | Хранит навыки                                        |
...
- users: 1,4 + 0,002 + 82,6 = 84 rps
- companies: 2138 + 632 + 0,02 = 2770
- vacancies: 2138 + 632 + 0,02 + 0,004 + 9,6 + 9,6 = 2789
- applications: 0,004 + 9,6 + 9,6 + 82,6 = 100
- resumes: 9,6 + 9,6 + 0,006 = 20
...
- users:
  ...
  - Всего: 225 * 54 000 000 = 967300000 байт = 11,3 Гб

- companies:
  - 1 строка: 8 + 40 + 200 + 60 + 40 + 60 + 8 + 8 + 1 = 425 байт
  - Всего: 425 * 2 276 000 = 967300000 байт = 0,9 Гб
    
- company_recruiters:
  - 1 строка: 8 + 8 + 8 + 40 + 60 + 10 + 8 = 142 байта
  - Всего: 142 * 54 000 000 = 7668000000 байт = 7,14 Гб

- vacancies:
  - 1 строка: 8 + 8 + 8 + 40 + 60 + 40 + 60 + 300 + 4 + 4 + 20 + 40 + 8 + 8 + 1 + 1 = 610 байт
  - Всего: 610 * 1 217 000 = 742370000 байт = 0,69 Гб

- resumes:
  - 1 строка: 8 + 8 + 30 + 200 + 300 + 40 + 200 + 8 + 8 + 1 = 803 байта
  - Всего: 803 * 88 000 000 = 70664000000 байт = 65,8 Гб
...
- applications: партиционирование по дате создания отклика (поле `created_at`)
...
|--------------------------|--------------------------------------------------------------------|-------------------------------------------------------------------------------|
| **Docker**               | Деплой и масштабирование                                           | Создание контейнеров для деплоя веб-приложения в изолированных средах, работает с k8s          |
| **Kubernetes**           | Деплой и масштабирование                                           | Обеспечивает автоматизацию масштабирования, отказоустойчивость и управление ресурсами          |
| **Nginx**                | Балансировка L7                                                    | Легкость в конфигурации, высокая пропускная способность, возможности кэширования и терминации SSL|
| **PostgreSQL**           | БД                                          | Реляционная СУБД с широким набором функциональных возможностей                |
| **ElasticSearch**	       | БД	| Мощный движок для полнотекстового поиска, обеспечивающий быстрый и эффективный поиск данных по ключевым словам и заданным фильтрам. |
| **Minio**                | БД                           | Высокопроизводительная объектная система хранения с открытым исходным кодом            |
| **gRPC**                 | Backend                     | Позволяет эффективно и быстро передавать данные между сервисами, снижая накладные расходы     |
| **Golang**               | Backend                                          | Высокая скорость компиляции и исполнения, а также строгая типизация обеспечивают надежность    |
| **Prometheus + Grafana** | Backend                               | Широко распространенное эффективное решение для сбора метрик и удобной визуализации данных                            |
| **Logstash**             | Backend                                   | Инструмент для сбора, обработки и передачи логов и событий.  |
| **TypeScript**           | Frontend                                                          | Язык для написания фронтенда, эффективен засчёт статической типизации          |
| **React**                | Frontend                              | Позволяет быстро создавать динамические и масштабируемые интерфейсы благодаря компонентной архитектуре |
| **Webpack**              | Frontend                                          | Позволяет оптимизировать загрузку ресурсов и улучшить производительность SPA   |
| **Swift**                | Frontend (iOS)                                                     | Интуитивно понятный и мощный язык для создания нативных приложений под iOS    |
| **Kotlin**               | Frontend (Android)                                                 | Обеспечивает простую и современную разработку Android-приложений              |
| **GitLab**               | Система контроля версий, CI/CD                                     | Популярная платформа для автоматизации CI/CD процессов и управления версиями  |

## 9. Обеспечение надёжности

| Аспект системы         | Обеспечение надежности                                                                     |
|------------------------|--------------------------------------------------------------------------------------------| 
| Nginx                  | Запуск нескольких экземпляров для отказоустойчивости, rate limit, health checks            | 
| Kubernetes             | Регулярные health checks, менеджмент подаов replicaset/statefulset              | 
| S3                     | S3 Sandbox - отдельный бакет, в который помещаются непроверенные файлы перед тем, как положить этот файл в конечный S3 бакет    |
| Мониторинг             | Prometheus + Grafana                                              |

#### Физический уровень

* Резервирование электропитания: использование ИБП, дизель-генераторов, двух независимых вводов питания, автоматическое переключение при сбоях.
* Резервирование сетевой инфраструктуры: дублирование каналов связи, отказоустойчивые маршрутизаторы/коммутаторы, независимые трассы кабелей.
* Мониторинг нагрузки для избегания перегрузки линий.
* Системы обнаружения перегрева, кондиционирование, защита от пожара, контроль пыли и влажности.


#### Graceful shutdown

* Плавная остановка сервиса, позволяющая обработать оставшиеся запросы перед заверщением (при этом перестает принимать новые запросы)
* Получаем SIGTERM -> новые запросы не принимаются -> обрабатываются необработанные запросы -> закрываются подключения -> завершение работы

#### Graceful degradation

* При возникновении проблем или перегрузки система не падает полностью, а продолжает работать, но с пониженным качеством или функциональностью, сохраняя основной сервис доступным.
* При дефиците ресурсов LLM модель отключается или работает с пониженной частотой вызовов, отдавая приоритет основной логике откликов.
...
| Действие             | RPS пик | Таблицы | 
|----------------------|---------|------------------------|
| Авторизация                 | 1,4 | users, company_recruiters |
| Регистрация                 | 0,002 | users, company_recruiters |
| Просмотр вакансий           | 2138 | vacancies, companies |
| Поиск вакансий              | 632 | vacancies, companies | 
| Публикация вакансий         | 0,02 | vacancies, companies, company_recruiters | 
| Закрытие вакансий           | 0,004 | vacancies, applications | 
| Отклик на вакансию          | 9,6 | applications, vacancies, resumes | 
| Ответ на отклик на вакансию | 9,6 | applications, vacancies, resumes | 
| Использование чата          | 82,6 | users, applications, messages | 
| Создание резюме             | 0,006 | resumes | 
...
| Таблица             | RPS пик | CPU Общ | CPU ДЦ1 | CPU ДЦ2 | CPU ДЦ3 |
|---------------------|---------|---------|---------|---------|---------|
| users               | 84 | 3 | 1 | 1 | 1 |
| companies           | 2770 | 12 | 4 | 4| 4 |
| vacancies           | 2789 | 12 | 4 | 4| 4 |
| applications        | 100 | 3 | 1 | 1 | 1 | 
| resumes             | 20 | 3 | 1 | 1 | 1 |
| messages            | 82,6 | 3 | 1 | 1 | 1 |
| company_recruiters  | 1,4 | 3 | 1 | 1 | 1 |
| es_vacancies        | 632 | 15 | 5 | 5 | 5 |
| фото                | 2771| 30 | 10 | 10 | 10 |

Поделим на коэфф 0,4: 84/0,4 ~= 210 CPU (по 70 ядер на каждый ДЦ)

### Хранилище

|Таблица  | Вес таблицы   | Вес индекса |
|--------------|-------|-----|
|users   |11,3 Гб Гб| 1,3 Гб |
|companies          |0,9 Гб|
|company_recruiters       |7,14 Гб| 1,2 Гб |
|vacancies       |0,69 Гб| 72 Мб |
|resumes  |65,8 Гб| 2 Гб |
|applications|999,9 Гб| 116,9 Гб |
|messages|1186,9 Гб| 116,4 Гб |
| **Итого** | 2272,6 Гб | 238,7 Гб |

Minio: 4445 ГБ (фото в компаниях) + 406 Гб (фото в вакансиях) + 7724 Гб (фото пользователей) + 11 000 Гб (фото резюме) = 23 575 Гб

Применим коэфф 0,4 и получим 5500 Гб и 600 Гб для БД и 70 ТБ для Minio

### Сервисы

Рассчитаем нагрузку для действий, написанных на Go
...
| Действие         | Пиковый RPS    | Тип бизнес-логики | CPU               | RAM, ГБ           |
| -------------- | ---------------- | ------------------| ----------------- | ----------------- | 
| Авторизация                   | 1,4       | лёгкая         | 1                 | 0,1          |
| Регистрация                   | 0,002 | средняя            | 1                 | 0,5          |
| Просмотр вакансий             | 2138  | легкая             | 5                 | 0,5              |
| Поиск вакансий                | 632   | легкая             | 2                 | 0,2               |
| Публикация вакансий           | 0,02  | средняя            | 1                 | 0,5               
| Закрытие вакансий             | 0,004 | легкая             | 1                 | 0,1               |
| Отклик на вакансию            | 9,6   | средняя            | 1                 | 0,5               |
| Ответ на отклик на вакансию   | 9,6 | средняя              | 1                 | 0,5               |
| Использование чата            | 82,6 | средняя             | 1                 | 0,5               |
| Cоздание резюме               | 0,006    | среднее         | 1                 | 0,5               |
| **Итого**                     | **2873** | -               | **15/0,4 = 37,5** | **3,9/0,4 = 10** |

### Расходы

На 1 ДЦ приходится:
* 70 (БД) + 13 (сервисы) = 83 CPU
* 600 Гб (индексы) + 10 Гб (сервисы) = 610 Гб ОЗУ
* 5500 Гб (БД) + 23575 Гб (Minio) = 30 Тб

Сервер iRU Rock g2212ag4, 2U 2146473 [^26] (1 700 000 руб):
* 64 CPU
* 256 Гб ОЗУ + 4 плашки по 32 Гб [^28] (4 * 50 000 руб = 200 000 руб)
* Используя RAID 5, возьмем 4 SSD (1 отнимает RAID 5) по 12 Тб [^27] = 36 Тб (320 000 руб * 4 = 1 280 000 руб)
* Итого: 3 200 000 руб

На каждый ДЦ возьмем по 2 таких сервера: 3200000 * (2 * 3 + 1 (резервирование в случае отказа)) = 22 400 000 руб

Для резервирования SSD в случае поломки берем еще 2 SSD: 320 000 * 2 = 640 000 руб

Итого: 22 400 000 руб + 640 000 руб = 23 100 000 руб

* Сервер на аренду для сравнения: сервер selectel - PCL37-NVMe-25GE - 216 000 / месяц [^29]
* Наш сервер: 350 000 (аренда за стойку)/7 + 225 000 (за дополнительные 10кв) / 7 ~= 83 000 руб/мес
* Разница: 216 000 - 83 000 = 133 000 руб/мес
* 3 200 000 / 133 000 = 24 месяца -> по прошествии этого времени мы выходим в плюс в сравнении с арендой  
...
[^26]: https://www.citilink.ru/product/server-iru-rock-g2212ag4-2x9554-4x64gb-9361-8i-sgl-2x2200w-w-o-os-2146-2146473/properties/
