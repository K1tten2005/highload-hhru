...
<img width="1042" height="694" alt="image" src="assets/network_datacentres.png" />  

- Москва
  - 92% трафика исходит из России. В Москве сосредоточены крупнейшие российские IX и точки взаимодействия, и через них проходит основная внутрироссийская и международная маршрутизация; оттуда проще обеспечивать низкий RTT и много путей к операторам.[^20]
- Санкт-Петербург
  - latency между Москвой и Санкт-Петербургом составляет примерно 10 мс, что является очень маленькой задержкой. Расположим там ДЦ для обеспечения отказоустойчивости в Центральной федеральнмом округе РФ.
- Новосибирск
  - Датацентр в Новосибирске нужен, чтобы снизить задержку для пользователей за Уралом и обеспечить географическую отказоустойчивость, а также обеспечить приемлемые условия эксплуатации сайта для жителей Дальнего Востока
 

DNS балансировка Latency based DNS, чтобы сетевой трафик распределялся географически

Anycast использовать нет нужды, посколько трафик в основном исходит из России и Беларуси, т.е. нет глобальной актуальности в сервисе.
...
- L4 балансировка. Используем Virtual Server via IP Tunneling (на LVS). Это решение справляется с проблемой большого объёма исходящего трафика, который при проксировании через NAT делает балансировщик "горлышком бутылки", ведь через балансировщик в этом подходе отдается овтет от сервера. В подходе через ip туннель ip пользователя инкапсулируется в пакет и через ip туннель передается от балансировщика к серверу, что позволяет серверу отправить ответ непосредственно к пользователю, избегая балансировщик. Также для балансировщика запускаем отдельный сервис keepalived для мониторинга "жизни" балансировщика. Реализует протокол VRRP (Virtual Router Redundancy Protocol), который позволяет виртуальный ip балансировщика передать другому балансировщику, если текущий активный балансировщик отвалится
- L7 балансировка. Используем HTTP Reverse proxy (Nginx). Прокси, являющийся прослойкой между L4 балансировщиком и backend-сервисом. Реализовывает SSL Termination, чтобы внутренние сервисы работали по http, а не https, т.е. https запрос расшифровывается на L7 прокси. Также может хранить часто запрашиваемый контент и тем самым ускоряет отклик.
