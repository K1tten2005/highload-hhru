...
- L7 балансировка. Используем HTTP Reverse proxy (Nginx). Реализовывает SSL Termination, чтобы внутренние сервисы работали по http, а не https, т.е. https запрос расшифровывается на L7 прокси. Также может хранить часто запрашиваемый контент и тем самым ускоряет отклик. Будем использовать 2N балансировщиков, т.е. 2 балансировщика, 1 основной и 1 резервирующий. Также нужен отдельный сервис keepalived для мониторинга жизни. Через протокол VRRP при отказе осноного балансировщика траффик будет идти на резервирующий.


## 5. Логическая схема БД

<img width="1323" height="702" alt="image" src="https://github.com/user-attachments/assets/b244f5f6-456c-47d3-9ddb-95544bfcd1ba" />
...
- Пользователи (users)
  ...
  - Email уникален.
  - Удаление запрещено при наличии активных откликов или чатов.
...
- Отклики (applications)
  ...
  - Все изменения статуса выполняются транзакционно.
