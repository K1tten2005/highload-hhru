...
- L7 балансировка. Используем HTTP Reverse proxy (Nginx). Реализовывает SSL Termination, чтобы внутренние сервисы работали по http, а не https, т.е. https запрос расшифровывается на L7 прокси. Также может хранить часто запрашиваемый контент и тем самым ускоряет отклик.
