...
| Создание резюме             | 0,003 | 0,006 | 0,009 | 0,047 |
...
- Создание резюме
  ...
  - Размер резюме рассчитывался в прошлом пункте: 5,8 Кб (текст) + 200 Кб (фото) * 0,865 = 178,8 Кб
  - 0,006 * 178,8 Кб * 8 / 1024 = 0,009 Мбит/сек
  - 0,003 * 178,8 Кб * 86400 / (1024 * 1024) = 0,047 Гбайт/сутки
...
<img width="1042" height="694" alt="image" src="network_datacentres.png" />  
...
- Новосибирск
  - Датацентр в Ноосибирске нужен, чтобы снизить задержку для пользователей за Уралом и обеспечить географическую отказоустойчивость
- Владивосток
  - Для минимизации задержки для запросов из Камчатского края


Схема DNS балансировки: latency-based
